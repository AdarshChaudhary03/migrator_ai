# Agent Report: scanner_analyzer_agent

Generated: 2025-11-17T11:37:43.027340 UTC
Repository: spring-boot-explorer

## Before: scan_spring_boot_features

| Key | Value |
| --- | --- |
| repo_path | "/tmp/migration-output/spring-boot-explorer" |

## After: scan_spring_boot_features

```json
{
  "success": true,
  "repo_path": "/tmp/migration-output/spring-boot-explorer",
  "scan_timestamp": "2025-11-17T17:07:46.109841",
  "overall_risk_score": 0.2636363636363636,
  "total_modules": 1,
  "total_features_found": 3,
  "summary_features": [
    {
      "name": "spring-boot-starter-web",
      "type": "spring_starter",
      "count": 1,
      "locations": [
        "/tmp/migration-output/spring-boot-explorer/pom.xml"
      ],
      "risk_score": 0.3,
      "migration_notes": "Web MVC starter - needs JAX-RS conversion"
    },
    {
      "name": "spring-boot-starter-test",
      "type": "spring_starter",
      "count": 1,
      "locations": [
        "/tmp/migration-output/spring-boot-explorer/pom.xml"
      ],
      "risk_score": 0.2,
      "migration_notes": "Test starter - convert to @QuarkusTest"
    },
    {
      "name": "@RestController",
      "type": "spring_annotation",
      "count": 1,
      "locations": [
        "/tmp/migration-output/spring-boot-explorer/src/main/java/com/lahcencodes/springbootexplorer/SpringBootExplorerApplication.java"
      ],
      "risk_score": 0.3,
      "migration_notes": "REST controller - convert to JAX-RS @Path"
    }
  ],
  "modules": [
    {
      "module_name": "spring-boot-explorer",
      "module_path": "/tmp/migration-output/spring-boot-explorer",
      "build_tool": "maven",
      "risk_score": 0.2636363636363636,
      "feature_count": 3,
      "features": [
        {
          "name": "spring-boot-starter-web",
          "type": "spring_starter",
          "count": 1,
          "risk_score": 0.3,
          "locations": [
            "/tmp/migration-output/spring-boot-explorer/pom.xml"
          ],
          "migration_notes": "Web MVC starter - needs JAX-RS conversion"
        },
        {
          "name": "spring-boot-starter-test",
          "type": "spring_starter",
          "count": 1,
          "risk_score": 0.2,
          "locations": [
            "/tmp/migration-output/spring-boot-explorer/pom.xml"
          ],
          "migration_notes": "Test starter - convert to @QuarkusTest"
        },
        {
          "name": "@RestController",
          "type": "spring_annotation",
          "count": 1,
          "risk_score": 0.3,
          "locations": [
            "/tmp/migration-output/spring-boot-explorer/src/main/java/com/lahcencodes/springbootexplorer/SpringBootExplorerApplication.java"
          ],
          "migration_notes": "REST controller - convert to JAX-RS @Path"
        }
      ],
      "dependency_info": {
        "success": true,
        "dependencies": {
          "groupId": "com.lahcencodes",
          "artifactId": "springbootexplorer",
          "version": "0.0.1-SNAPSHOT",
          "type": "jar",
          "scope": "",
          "classifier": "",
          "optional": "false",
          "children": [
            {
              "groupId": "org.springframework.boot",
              "artifactId": "spring-boot-starter-web",
              "version": "3.4.0",
              "type": "jar",
              "scope": "compile",
              "classifier": "",
              "optional": "false",
              "children": [
                {
                  "groupId": "org.springframework.boot",
                  "artifactId": "spring-boot-starter",
                  "version": "3.4.0",
                  "type": "jar",
                  "scope": "compile",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "org.springframework.boot",
                      "artifactId": "spring-boot",
                      "version": "3.4.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.springframework.boot",
                      "artifactId": "spring-boot-autoconfigure",
                      "version": "3.4.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.springframework.boot",
                      "artifactId": "spring-boot-starter-logging",
                      "version": "3.4.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false",
                      "children": [
                        {
                          "groupId": "ch.qos.logback",
                          "artifactId": "logback-classic",
                          "version": "1.5.12",
                          "type": "jar",
                          "scope": "compile",
                          "classifier": "",
                          "optional": "false",
                          "children": [
                            {
                              "groupId": "ch.qos.logback",
                              "artifactId": "logback-core",
                              "version": "1.5.12",
                              "type": "jar",
                              "scope": "compile",
                              "classifier": "",
                              "optional": "false"
                            }
                          ]
                        },
                        {
                          "groupId": "org.apache.logging.log4j",
                          "artifactId": "log4j-to-slf4j",
                          "version": "2.24.1",
                          "type": "jar",
                          "scope": "compile",
                          "classifier": "",
                          "optional": "false",
                          "children": [
                            {
                              "groupId": "org.apache.logging.log4j",
                              "artifactId": "log4j-api",
                              "version": "2.24.1",
                              "type": "jar",
                              "scope": "compile",
                              "classifier": "",
                              "optional": "false"
                            }
                          ]
                        },
                        {
                          "groupId": "org.slf4j",
                          "artifactId": "jul-to-slf4j",
                          "version": "2.0.16",
                          "type": "jar",
                          "scope": "compile",
                          "classifier": "",
                          "optional": "false"
                        }
                      ]
                    },
                    {
                      "groupId": "jakarta.annotation",
                      "artifactId": "jakarta.annotation-api",
                      "version": "2.1.1",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.yaml",
                      "artifactId": "snakeyaml",
                      "version": "2.3",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "org.springframework.boot",
                  "artifactId": "spring-boot-starter-json",
                  "version": "3.4.0",
                  "type": "jar",
                  "scope": "compile",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "com.fasterxml.jackson.core",
                      "artifactId": "jackson-databind",
                      "version": "2.18.1",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false",
                      "children": [
                        {
                          "groupId": "com.fasterxml.jackson.core",
                          "artifactId": "jackson-annotations",
                          "version": "2.18.1",
                          "type": "jar",
                          "scope": "compile",
                          "classifier": "",
                          "optional": "false"
                        },
                        {
                          "groupId": "com.fasterxml.jackson.core",
                          "artifactId": "jackson-core",
                          "version": "2.18.1",
                          "type": "jar",
                          "scope": "compile",
                          "classifier": "",
                          "optional": "false"
                        }
                      ]
                    },
                    {
                      "groupId": "com.fasterxml.jackson.datatype",
                      "artifactId": "jackson-datatype-jdk8",
                      "version": "2.18.1",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "com.fasterxml.jackson.datatype",
                      "artifactId": "jackson-datatype-jsr310",
                      "version": "2.18.1",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "com.fasterxml.jackson.module",
                      "artifactId": "jackson-module-parameter-names",
                      "version": "2.18.1",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "org.springframework.boot",
                  "artifactId": "spring-boot-starter-tomcat",
                  "version": "3.4.0",
                  "type": "jar",
                  "scope": "compile",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "org.apache.tomcat.embed",
                      "artifactId": "tomcat-embed-core",
                      "version": "10.1.33",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.apache.tomcat.embed",
                      "artifactId": "tomcat-embed-el",
                      "version": "10.1.33",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.apache.tomcat.embed",
                      "artifactId": "tomcat-embed-websocket",
                      "version": "10.1.33",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "org.springframework",
                  "artifactId": "spring-web",
                  "version": "6.2.0",
                  "type": "jar",
                  "scope": "compile",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "org.springframework",
                      "artifactId": "spring-beans",
                      "version": "6.2.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "io.micrometer",
                      "artifactId": "micrometer-observation",
                      "version": "1.14.1",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false",
                      "children": [
                        {
                          "groupId": "io.micrometer",
                          "artifactId": "micrometer-commons",
                          "version": "1.14.1",
                          "type": "jar",
                          "scope": "compile",
                          "classifier": "",
                          "optional": "false"
                        }
                      ]
                    }
                  ]
                },
                {
                  "groupId": "org.springframework",
                  "artifactId": "spring-webmvc",
                  "version": "6.2.0",
                  "type": "jar",
                  "scope": "compile",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "org.springframework",
                      "artifactId": "spring-aop",
                      "version": "6.2.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.springframework",
                      "artifactId": "spring-context",
                      "version": "6.2.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.springframework",
                      "artifactId": "spring-expression",
                      "version": "6.2.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                }
              ]
            },
            {
              "groupId": "org.springframework.boot",
              "artifactId": "spring-boot-starter-test",
              "version": "3.4.0",
              "type": "jar",
              "scope": "test",
              "classifier": "",
              "optional": "false",
              "children": [
                {
                  "groupId": "org.springframework.boot",
                  "artifactId": "spring-boot-test",
                  "version": "3.4.0",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false"
                },
                {
                  "groupId": "org.springframework.boot",
                  "artifactId": "spring-boot-test-autoconfigure",
                  "version": "3.4.0",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false"
                },
                {
                  "groupId": "com.jayway.jsonpath",
                  "artifactId": "json-path",
                  "version": "2.9.0",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "org.slf4j",
                      "artifactId": "slf4j-api",
                      "version": "2.0.16",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "jakarta.xml.bind",
                  "artifactId": "jakarta.xml.bind-api",
                  "version": "4.0.2",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "jakarta.activation",
                      "artifactId": "jakarta.activation-api",
                      "version": "2.1.3",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "net.minidev",
                  "artifactId": "json-smart",
                  "version": "2.5.1",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "net.minidev",
                      "artifactId": "accessors-smart",
                      "version": "2.5.1",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false",
                      "children": [
                        {
                          "groupId": "org.ow2.asm",
                          "artifactId": "asm",
                          "version": "9.6",
                          "type": "jar",
                          "scope": "test",
                          "classifier": "",
                          "optional": "false"
                        }
                      ]
                    }
                  ]
                },
                {
                  "groupId": "org.assertj",
                  "artifactId": "assertj-core",
                  "version": "3.26.3",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "net.bytebuddy",
                      "artifactId": "byte-buddy",
                      "version": "1.15.10",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "org.awaitility",
                  "artifactId": "awaitility",
                  "version": "4.2.2",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false"
                },
                {
                  "groupId": "org.hamcrest",
                  "artifactId": "hamcrest",
                  "version": "2.2",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false"
                },
                {
                  "groupId": "org.junit.jupiter",
                  "artifactId": "junit-jupiter",
                  "version": "5.11.3",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "org.junit.jupiter",
                      "artifactId": "junit-jupiter-api",
                      "version": "5.11.3",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false",
                      "children": [
                        {
                          "groupId": "org.opentest4j",
                          "artifactId": "opentest4j",
                          "version": "1.3.0",
                          "type": "jar",
                          "scope": "test",
                          "classifier": "",
                          "optional": "false"
                        },
                        {
                          "groupId": "org.junit.platform",
                          "artifactId": "junit-platform-commons",
                          "version": "1.11.3",
                          "type": "jar",
                          "scope": "test",
                          "classifier": "",
                          "optional": "false"
                        },
                        {
                          "groupId": "org.apiguardian",
                          "artifactId": "apiguardian-api",
                          "version": "1.1.2",
                          "type": "jar",
                          "scope": "test",
                          "classifier": "",
                          "optional": "false"
                        }
                      ]
                    },
                    {
                      "groupId": "org.junit.jupiter",
                      "artifactId": "junit-jupiter-params",
                      "version": "5.11.3",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.junit.jupiter",
                      "artifactId": "junit-jupiter-engine",
                      "version": "5.11.3",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false",
                      "children": [
                        {
                          "groupId": "org.junit.platform",
                          "artifactId": "junit-platform-engine",
                          "version": "1.11.3",
                          "type": "jar",
                          "scope": "test",
                          "classifier": "",
                          "optional": "false"
                        }
                      ]
                    }
                  ]
                },
                {
                  "groupId": "org.mockito",
                  "artifactId": "mockito-core",
                  "version": "5.14.2",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "net.bytebuddy",
                      "artifactId": "byte-buddy-agent",
                      "version": "1.15.10",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false"
                    },
                    {
                      "groupId": "org.objenesis",
                      "artifactId": "objenesis",
                      "version": "3.3",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "org.mockito",
                  "artifactId": "mockito-junit-jupiter",
                  "version": "5.14.2",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false"
                },
                {
                  "groupId": "org.skyscreamer",
                  "artifactId": "jsonassert",
                  "version": "1.5.3",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "com.vaadin.external.google",
                      "artifactId": "android-json",
                      "version": "0.0.20131108.vaadin1",
                      "type": "jar",
                      "scope": "test",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "org.springframework",
                  "artifactId": "spring-core",
                  "version": "6.2.0",
                  "type": "jar",
                  "scope": "compile",
                  "classifier": "",
                  "optional": "false",
                  "children": [
                    {
                      "groupId": "org.springframework",
                      "artifactId": "spring-jcl",
                      "version": "6.2.0",
                      "type": "jar",
                      "scope": "compile",
                      "classifier": "",
                      "optional": "false"
                    }
                  ]
                },
                {
                  "groupId": "org.springframework",
                  "artifactId": "spring-test",
                  "version": "6.2.0",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false"
                },
                {
                  "groupId": "org.xmlunit",
                  "artifactId": "xmlunit-core",
                  "version": "2.10.0",
                  "type": "jar",
                  "scope": "test",
                  "classifier": "",
                  "optional": "false"
                }
              ]
            }
          ]
        },
        "build_tool": "maven"
      }
    }
  ],
  "migration_complexity": "LOW",
  "recommended_approach": [
    "Convert REST controllers to JAX-RS resources",
    "Replace @RequestMapping with JAX-RS annotations"
  ]
}
```

